...
def changesmpltomesh_cam(X, camera,pred_cam_t):
    """Perform orthographic projection of 3D points X using the camera parameters
    Args:
        X: size = [B, N, 3]
        camera: size = [B, 3]
    ...
    """
    camera = camera.view(-1, 1, 3)
    X_trans = X + pred_cam_t.view(-1, 1, 3)
    shape = X_trans.shape
    X_3d = (camera[:, :, 0] * X_trans.reshape(shape[0], -1)).reshape(shape)
    return X_3d

def perspective_projection(points, rotation, translation,
                           focal_length, camera_center, retain_z=True):
    """
