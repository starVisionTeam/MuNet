...
import numpy as np
from  networks.graph_cmr.models.resnet import resnet50
savepath='/media/star/dataset_SSD/code/Cloth-shift/write_pic/thuman2_test/network/resnet50'
...
def draw_features(width,height,x,savename):
    ...
    for i in range(width*height):
        ...
        pmin = np.min(img)
        pmax = np.max(img)
        # cv2.imwrite(savename[:-4] + str(i) + '.png', img)
    ...
    plt.savefig(savename[:-4]+'feature_map_sum.png') # 保存图像到本地
    plt.show()
...
class ft_net(nn.Module):
    ...
    def forward(self, x):
        if True: # draw features or not
            ...
            print(x.size)
            draw_features(32,64, x.cpu().numpy()[:, 0:2048, :, :], "{}/f9.png".format(savepath))
            # draw_features(32, 64, x.cpu().numpy()[0, :, 0, 0], "{}/f9.png".format(savepath))
            ...
            plt.plot(np.linspace(1, 2048, 2048), x.cpu().numpy()[0, :])
...
model.eval()
img=cv2.imread('/media/star/dataset_SSD/code/Cloth-shift/write_pic/thuman2_test/network/0033.jpg')
img=cv2.resize(img,(224,224))
img=cv2.rectangle(img,(112,142),(127,160),(0,255,255),1)
# cv2.imwrite('/media/star/dataset_SSD/code/Cloth-shift/write_pic/thuman2_test/network/0033.jpg',img)
img=cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
# img=img[142:161,112:128]
# cv2.imshow("img",img)
# cv2.waitKey(0)
