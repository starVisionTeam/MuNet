...
from networks.dataloader.util import load_obj, export
import copy
...
class Mesh2:
    def __init__(self, file=None, hold_history=False, vs=None, faces=None, device='cuda', gfmm=False):
        if file is None and vs is None:
            return
        if file is not None:
        ...
        if vs is not None and faces is not None:
            self.vs, self.faces = vs.squeeze().detach().cpu().numpy(), faces
        ...
        self.faces = self.faces.to(self.device).long().detach()
    ...
    def build_gemm(self):
        ...
        self.edges = np.array(torch.tensor(edges).cpu(), dtype=np.int32)
        self.gemm_edges = np.array(torch.tensor(edge_nb).cpu(), dtype=np.int64)
    ...
    # def update_verts2(self, new_vs: torch.Tensor, index: int):
    #     m = self.
    ...
    def from_tensor(mesh, vs, faces, gfmm=True):
        mesh = Mesh2(file=None, vs=vs, faces=faces, device=mesh.device, hold_history=True, gfmm=gfmm)
        return mesh
...
class PartMesh:
    ...
    def __init__(self, main_mesh: Mesh2, vs_groups=None, num_parts=1, bfs_depth=0, n=-1):
        """
        ...
        for i in range(self.n_submeshes):
            ...
            vs_index = torch.tensor(self.vs_bfs(vs_index.tolist(), self.main_mesh.faces.tolist(), bfs_depth),
                                    dtype=vs_index.dtype).to("cuda")
            m, vs_index = self.main_mesh.submesh(vs_index)
            # print(vs_index)
            self.sub_mesh.append(m)
            self.sub_mesh_index.append(vs_index.cpu().numpy())
        ...
        for i in range(self.n_submeshes):
            ...
            for face in self.sub_mesh[i].faces:
                ...
                face = torch.tensor(self.sub_mesh_index[i][face.cpu()]).to(face.device).long()
                for j in range(3):
    ...
    def __getitem__(self, i: int) -> Mesh2:
        """
    ...
    @staticmethod
    def create_submesh(vs_index: torch.Tensor, mesh: Mesh2) -> (Mesh2, torch.Tensor):
        """
        ...
        """
        vs_mask = torch.zeros(mesh.vs.shape[0]).to(mesh.faces.device)
        vs_mask[vs_index] = 1
        ...
        m = Mesh2.from_tensor(mesh, new_vs.detach(), new_faces.detach(), gfmm=False)
